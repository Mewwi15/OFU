 ...
 *
 * ★ กระดาษ 48 มม. ต้องไม่โดนคำเตือนนี้ ★ คำเตือนมีไว้กันเอกสารใหญ่ไหลลงม้วนใบเสร็จ
 * เท่านั้น แต่ของเดิมเตือนทุกอย่างที่ผ่านฟังก์ชันนี้ รวมสลิปของหน้ารอบขายซึ่ง "ตั้งใจ"
 * ให้ออกม้วนใบเสร็จอยู่แล้ว — แคชเชียร์อ่านแล้วกด "ยกเลิก" ตามที่กล่องชวน ผลคือลิ้นชัก
 * ไม่เปิด ทั้งที่ระบบบันทึกไปแล้วว่าเปิด · และการไปถามตัวกลางก่อนยังหน่วงอีก 1.2 วินาที
 * ต่อสลิปตอนลูกค้ายืนรอแลกแบงก์ ทั้งที่คำตอบไม่ได้เปลี่ยนเส้นทางพิมพ์เลย
 */
export function printHtml(html: string, paper: 'a4' | 'thermal' = 'a4') {
  void (async () => {
    if (paper === 'thermal') {
      openPrintFrame(html);
      return;
