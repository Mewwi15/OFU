...
/* ── review videos (แถบรีวิวสินค้าบนหน้าแรกของแอป — 0099) ────────────────────── */
...
/** ทุกคลิปรวมทั้งฉบับร่าง — RLS ปล่อยให้แอดมินของร้านเห็นทั้งหมด */
export async function listReviewVideos(): Promise<ReviewVideo[]> {
  const { data, error } = await supabase
    .from('review_videos')
    .select('id, video_path, poster_path, caption, product_id, display_order, publish_state')
    .order('display_order');
  if (error) throw error;
  return data as ReviewVideo[];
}

export const upsertReviewVideo = (p: {
  id?: string;
  video_path: string;
  poster_path?: string | null;
  caption?: string | null;
  ...
  rpc<{ id: string }>('upsert_review_video', {
    p_id: p.id ?? undefined,
    p_video_path: p.video_path,
    p_poster_path: p.poster_path ?? undefined,
    p_caption: p.caption ?? undefined,
    p_product_id: p.product_id ?? undefined,
    p_display_order: p.display_order ?? 0,
    p_publish_state: p.publish_state ?? 'draft',
  });

export const deleteReviewVideo = (id: string) => rpc('delete_review_video', { p_id: id });

/**
 * อัปคลิปขึ้นบักเก็ต review-videos แล้วคืน "path" ไม่ใช่ URL เต็ม
 *
 * ต่างจาก uploadBannerImage ที่คืน URL เต็มโดยตั้งใจ — ตารางรีวิวเก็บ path ล้วน
 * แล้วให้ฝั่งแอปประกอบ URL ตอนอ่าน (ดูเหตุผลใน 0099) ย้ายโปรเจกต์แล้วไม่ต้องไล่แก้ทั้งตาราง
 */
...
/** ภาพปกไปอยู่บักเก็ตรูปเดิม ไม่ต้องเปิดบักเก็ตใหม่ให้ซ้ำซ้อน — คืน path เช่นกัน */
export async function uploadReviewPoster(file: File): Promise<string> {
  const ext = file.name.split('.').pop()?.toLowerCase() || 'jpg';
  const path = `review-posters/${crypto.randomUUID()}.${ext}`;
  const { error } = await supabase.storage
    .from('product-images')
    .upload(path, file, { cacheControl: '3600', upsert: false, contentType: file.type || undefined });
  if (error) throw error;
...
/** path ในบักเก็ต → URL เต็มสำหรับพรีวิวในหน้าแอดมิน */
