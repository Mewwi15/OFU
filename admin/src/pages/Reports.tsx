      ...
      <Row gutter={[12, 12]} className="mb-1">
        <Col xs={12} lg={6}>
          <Card size="small" styles={{ body: { padding: '12px 16px' } }} loading={loading}>
            <Statistic title="ยอดขายรวม" value={totalGross} prefix="฿" valueStyle={{ color: '#14532D', fontWeight: 700 }} />
          </Card>
        </Col>
        <Col xs={12} lg={6}>
          <Card size="small" styles={{ body: { padding: '12px 16px' } }} loading={loading}>
          ...
          <Card title="ตามช่องทาง" loading={loading}>
            {data && (
              <div className="space-y-4">
                <ChannelBar Icon={RiStore2Line} label="หน้าร้าน (POS)" amount={data.onsite.gross} count={data.onsite.count} pct={pct(data.onsite.gross, totalGross)} stroke="#14532D" />
          ...
          columns={[
            {
              ...
              width: 48,
              render: (_, __, i) => (
                <Tag color="#F5F5F5" style={{ color: '#14532D', border: 'none', margin: 0 }}>{i + 1}</Tag>
              ),
            },
