...
/**
 * ★ สวิตช์เดียวที่คุมทางเข้าด้วยเบอร์ ★
 *
 * ปิดไว้ 6 ก.ย. 2026 ระหว่างรออนุมัติชื่อผู้ส่ง — ตอนยื่นขอชื่อ OFU ชื่อที่ยืมใช้อยู่เดิม
 * (SMS-DEMO) ถูกถอดออกจากบัญชีทันที = ส่ง SMS ไม่ได้เลย เปิดทางเข้าทิ้งไว้ก็มีแต่คนกด
 * แล้วรอรหัสที่ไม่มีวันมาถึง
 *
 * เปิดใช้ 18 ก.ย. 2026 หลังพิสูจน์กับเบอร์จริงว่าส่งเข้าแล้ว (ref O28cPnLQCrIoqXO)
 * ★ ที่เคยส่งไม่เข้าเป็นเรื่องรายเบอร์ ไม่ใช่ทั้งระบบ ★ เบอร์ที่เราใช้ทดสอบรัว ๆ หลายสิบ
 * ครั้งโดนขึ้นบัญชีดำของผู้ให้บริการ ส่วนเบอร์อื่นส่งเข้าปกติ — ถ้าลูกค้าคนไหนเจอแบบ
 * เดียวกัน จะขึ้นในหน้าตั้งค่าหลังร้าน "เบอร์ที่รับ SMS ไม่ได้" ให้โทรตามได้ และหน้านี้
 * บอกลูกค้าให้เข้าด้วย Google/อีเมลแทนไปก่อน
 */
      ...
      <ScrollView
        ...
        {step === 'form' && lineOnly ? (
          ...
          <>
            ...
            {/* ทางออกให้คนที่ไม่ได้รับ SMS — SMS ไม่ถึงได้จากหลายสาเหตุที่ลูกค้าแก้เองไม่ได้
                (สัญญาณ กล่องเต็ม ค่ายบล็อก ชื่อผู้ส่งยังไม่อนุมัติ) ถ้าไม่บอกทางอื่นไว้ตรงนี้
                เขาจะกดส่งซ้ำวนไปจนเลิกใช้ — และร้านก็เสียค่า SMS ทุกครั้งที่กด */}
            {method === 'phone' ? (
              <Text variant="caption" style={styles.methodHint}>
                ไม่ได้รับ SMS? กดย้อนกลับแล้วเข้าด้วยอีเมลหรือ Google ได้เลย
              </Text>
            ) : null}
