-- 0048_online_fee_settings.sql
-- อู้ฟู่ (Oofoo) — online (parcel) shipping fee moves into shop_settings.
-- Owner request 2026-07-12: parcel fee 40 → 150 THB. The fee was hardcoded
-- (flat 40, free >= 500) inside validate_promo + place_order (0005); this
-- migration adds settings columns (like the rider-mode fee already has) and
-- re-creates both functions to read them, so the next price change is an
-- UPDATE, not a migration. Free-shipping threshold stays 500. Grants carry
-- over (CREATE OR REPLACE keeps existing privileges).

alter table public.shop_settings
  add column if not exists online_fee int not null default 150 check (online_fee >= 0),
  add column if not exists online_free_threshold int not null default 500 check (online_free_threshold >= 0);

-- Rows created before this migration get the new fee explicitly.
update public.shop_settings set online_fee = 150, online_free_threshold = 500;
...
begin
  ...
  if v_promo.scope = 'delivery'::public.promo_scope_t then
    if p_shop_mode = 'online'::public.shop_mode_t then
      select online_fee, online_free_threshold into v_fee, v_threshold
      from public.shop_settings where shop_id = v_shop;
      v_base := case when v_subtotal >= coalesce(v_threshold, 500)
                     then 0 else coalesce(v_fee, 150) end;
    else
...
begin
  ...
  v_online_threshold := coalesce(v_online_threshold, 500);
  ...
  else  -- online: parcel fee from shop_settings (0048)
    v_fee := case when v_subtotal >= v_online_threshold then 0 else v_online_fee end;
  end if;
