-- 0019_pos_rpc.sql
-- อู้ฟู่ POS (P1) — RPCs: open_shift, close_shift, create_pos_sale.
-- SECURITY DEFINER, search_path=''. Guarded by admin_shop() (owner/staff/cashier).
-- create_pos_sale is atomic + idempotent (client_op_id) for offline sync replay,
-- decrements the SHARED product stock, backs VAT out of VAT-inclusive prices, and
-- assigns per-shop gapless receipt / tax-invoice numbers.

create or replace function public.open_shift(p_opening_float int default 0)
returns public.pos_shifts language plpgsql security definer set search_path = '' as $$
declare v_shop uuid := public.admin_shop(); v_row public.pos_shifts;
begin
  ...
  insert into public.pos_shifts (shop_id, cashier_user_id, opening_float)
  values (v_shop, auth.uid(), greatest(coalesce(p_opening_float, 0), 0))
...
returns public.pos_shifts language plpgsql security definer set search_path = '' as $$
declare v_shop uuid := public.admin_shop(); v_row public.pos_shifts; v_cash int; v_expected int;
begin
...
create or replace function public.create_pos_sale(
  p_client_op_id uuid,
  p_items jsonb,                                   -- [{variant_id, qty, line_discount?}]
...
declare
  ...
  v_shift uuid; v_ex public.pos_sales;
...
begin
  ...
  select id into v_shift from public.pos_shifts
   where shop_id = v_shop and cashier_user_id = v_cashier and closed_at is null limit 1;
  if v_shift is null then raise exception 'NO_OPEN_SHIFT' using errcode = 'P0001'; end if;
  if p_items is null or jsonb_array_length(p_items) = 0 then
    raise exception 'EMPTY_SALE' using errcode = 'P0001';
  end if;

  -- resolve lines (server price), lock variants in id order, check stock
  create temp table _lines (variant_id uuid, product_name text, size text,
    unit_price int, qty int, line_discount int, line_total int) on commit drop;
  for rec in
    select (i->>'variant_id')::uuid vid, (i->>'qty')::int qty, coalesce((i->>'line_discount')::int, 0) ld
    from jsonb_array_elements(p_items) i
    order by (i->>'variant_id')::uuid
  loop
    declare v_price int; v_stock int; v_name text; v_size text;
    begin
      select v.price, v.stock_qty, p.name, v.size into v_price, v_stock, v_name, v_size
      ...
      insert into _lines values (rec.vid, v_name, v_size, v_price, rec.qty, rec.ld, v_price * rec.qty - rec.ld);
  ...
  -- VAT backed out of a VAT-inclusive total
  ...
  values (v_shop, v_cashier, v_shift, 'POS' || to_char(v_saleno, 'FM000000'),
  ...
  -- items + decrement shared stock + ledger
  insert into public.pos_sale_items (sale_id, variant_id, product_name, size, unit_price, qty, line_discount)
  select v_sale, variant_id, product_name, size, unit_price, qty, line_discount from _lines;
...
revoke execute on function public.open_shift(int) from public;
revoke execute on function public.close_shift(uuid, int) from public;
revoke execute on function public.create_pos_sale(uuid, jsonb, public.pos_pay_method_t, int, int, uuid, text, text, boolean) from public;
grant execute on function public.open_shift(int) to authenticated;
grant execute on function public.close_shift(uuid, int) to authenticated;
