-- 0060_pos_sale_zero_total_payment.sql
-- อู้ฟู่ (Oofoo) — a full-discount POS sale (total = 0, e.g. a free-item
-- giveaway) crashed with a raw, untranslated Postgres error: "new row for
-- relation pos_sale_payments violates check constraint
-- pos_sale_payments_amount_check" (amount > 0). The single-tender branch of
-- create_pos_sale always inserted a payment row for v_total, even when it's
-- 0 — no money actually changed hands, so there's nothing to record. The
-- split-tender branch already filters out ฿0 legs (0029/0041: `where
-- (e->>'amount')::int > 0`); this brings the single-tender branch in line
-- with the same rule. Same signature, safe create-or-replace.
