-- 0049_no_free_online_shipping.sql
-- อู้ฟู่ (Oofoo) — online (parcel) shipping has NO free tier (owner, 2026-07-12,
-- minutes after 0048 set the fee to 150): every online order pays online_fee.
-- online_free_threshold becomes nullable; NULL = no free-shipping tier, and
-- both fee sites now treat it that way. Re-enable free shipping later by
-- setting a number again — no migration needed.

alter table public.shop_settings alter column online_free_threshold drop not null;
alter table public.shop_settings alter column online_free_threshold drop default;
update public.shop_settings set online_free_threshold = null;
...
begin
  ...
  if v_promo.scope = 'delivery'::public.promo_scope_t then
    if p_shop_mode = 'online'::public.shop_mode_t then
      ...
      v_base := case when v_threshold is not null and v_subtotal >= v_threshold
