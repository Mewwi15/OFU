...
-- ── 2. Partial index ─────────────────────────────────────────────────────────
-- The job runs every 5 minutes forever, so its scan must never be a seq scan
-- over a growing orders table. Predicate mirrors the function's filter exactly.
...
comment on function public.expire_stale_orders(timestamptz, int) is
  'Cancels prepay orders left unpaid past p_before and releases their reserved '
  'stock (reason release_expiry). Internal/cron only — never exposed to clients. '
  'Never touches COD, slip_uploaded/verifying, paid or terminal orders.';
